...
# This logic ensures values are quoted ONLY if they contain spaces, commas, or internal quotes,
# and that internal quotes are escaped correctly for gcloud's parser.
ENV_VARS_STRING=""
FIRST_VAR=true
for KEY in "${!ENV_VARS[@]}"; do
    ...
    NEEDS_QUOTING=false
    # Check if value contains space, comma, or a literal double quote
    ...
    if $NEEDS_QUOTING; then
        # Escape internal double quotes for gcloud's parser
        # 's/"/\\"/g' ensures " becomes \"
        PROCESSED_VALUE=$(echo "$PROCESSED_VALUE" | sed 's/"/\\"/g')
        # Wrap the entire value in double quotes for gcloud's parser
        PROCESSED_VALUE="\"${PROCESSED_VALUE}\"" # e.g., "value with spaces and \"quotes\""
    fi
    
    if [ "$FIRST_VAR" = true ]; then
        ENV_VARS_STRING="${KEY}=${PROCESSED_VALUE}"
        FIRST_VAR=false
    else
        ENV_VARS_STRING+=",${KEY}=${PROCESSED_VALUE}"
    fi
...
- name: 'gcr.io/cloud-builders/gcloud'
  ...
  - '--port'
  - "3000" # Changed from '3000' to "3000" for robustness
  - '--memory'
  - "${_MEMORY}"
  - '--cpu'
  - "${_CPU}" # Changed from '${_CPU}' to "${_CPU}" for robustness
  - '--min-instances'
  - "${_MIN_INSTANCES}" # Changed from '${_MIN_INSTANCES}' to "${_MIN_INSTANCES}" for robustness
  - '--max-instances'
  - "${_MAX_INSTANCES}" # Changed from '${_MAX_INSTANCES}' to "${_MAX_INSTANCES}" for robustness
  - '--timeout'
  - "${_TIMEOUT}" # Changed from '${_TIMEOUT}' to "${_TIMEOUT}" for robustness
  - '--concurrency'
  - "${_CONCURRENCY}" # Changed from '${_CONCURRENCY}' to "${_CONCURRENCY}" for robustness
  - '--set-env-vars'
  - "${ENV_VARS_STRING}" # This entire string must be a single line in the YAML
  - '--project'
  - '${PROJECT_ID}'
